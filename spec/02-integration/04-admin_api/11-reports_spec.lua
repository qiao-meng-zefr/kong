...
for _, strategy in helpers.each_strategy() do
  ...
  describe("#flaky anonymous reports in Admin API #" .. strategy, function()
    ...
    if strategy == "off" then
      it("#only reports declarative reconfigure via /config", function()
        ...
        assert.same(1, #reports_data)
        assert.match("signal=dbless%-reconfigure", reports_data[1])
        assert.match("_format_version=1.1", reports_data[1])
      end)
    end
